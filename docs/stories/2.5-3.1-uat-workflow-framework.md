...
[Source: CLAUDE.md#Testing, docs/TESTING-README.md]
...
**New Testing Infrastructure Available (Story 2.5.3)**:
- **Large document test fixtures**: `tests/fixtures/pdfs/large/audit-report-large.pdf` (60 pages), `tests/fixtures/xlsx/large/audit-data-10k-rows.xlsx` (10,240 rows), `tests/fixtures/pdfs/scanned/audit-scan.pdf` (5 pages image-based)
- **Integration tests**: `tests/integration/test_large_files.py` (4 tests: memory usage, Excel processing, OCR completion, memory monitoring accuracy)
- **Fixture generation scripts**: `scripts/generate_large_pdf_fixture.py`, `scripts/generate_large_excel_fixture.py`, `scripts/generate_scanned_pdf_fixture.py` (reusable patterns for future fixtures)
- **Fixture documentation**: `tests/fixtures/README.md` (comprehensive inventory, generation process, contributor guidelines)
- **Brownfield tracking**: `docs/brownfield-test-failures-tracking.md` (25 pre-existing test failures documented separately from greenfield work)
- [Source: stories/2.5-3-quality-gate-automation-and-documentation.md#File-List]

**Key Metrics and Patterns (Story 2.5.3)**:
- **NFR-P2 validated**: <2GB memory target met - Peak memory 167MB for 60-page PDF (92% under 2GB threshold)
- **Fixture size budget**: 35.87 MB / 100 MB used (64% margin remaining for future fixtures)
- **Reusable memory monitoring pattern**: `get_total_memory()` function from `scripts/profile_pipeline.py:151-167` (aggregates main + worker processes, 9.6ms overhead)
- **Quality gates**: Black ✅, Ruff ✅, Mypy ✅ (0 violations achieved)
- **Integration test results**: 4/4 PASS in 5.67s (test_large_pdf_memory_usage, test_large_excel_processing, test_scanned_pdf_ocr_completion, test_memory_monitoring_accuracy)
...
- [Source: docs/TESTING-README.md] - Testing standards and coverage requirements
...
- **2025-11-13**: Story quality validation and improvements (SM agent)
  - Added "New Testing Infrastructure Available" subsection to Learnings (8 NEW files from Story 2.5.3)
  - Added "Key Metrics and Patterns" subsection with NFR validation results and reusable patterns
  - Fixed invalid citation: testing-strategy.md → TESTING-README.md (lines 241, 351)
  - Validation result: All critical and major issues resolved
- **2025-11-13**: Story implementation completed (Dev agent)
  - All 4 UAT workflows created with complete file structures (20 files total)
  - Comprehensive audit of all workflows using audit-workflow (15 issues identified and fixed)
  - UAT framework documentation added to tech-spec-epic-2.5.md (767 lines)
  - CLAUDE.md updated with UAT workflow usage guide
  - Quality score: 90.75/100 average across all workflows
  - All acceptance criteria satisfied (AC-2.5.3.1-1 through AC-2.5.3.1-5)

## Dev Agent Record

### Context Reference

- docs/stories/2.5-3.1-uat-workflow-framework.context.xml
