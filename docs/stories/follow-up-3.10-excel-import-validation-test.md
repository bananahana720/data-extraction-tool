...
def test_excel_special_characters(tmp_path):
    ...
    chunk = create_chunk_with_special_chars(
        text='Text with "quotes", commas,\nand newlines. Emoji: 🧪. Chinese: 测试. Arabic: اختبار.'
    )
    ...
    assert '🧪' in chunk_text_cell
    assert '测试' in chunk_text_cell
    assert 'اختبار' in chunk_text_cell
...
- [ ] Traceability matrix updated (AC-3.6-4 coverage: PARTIAL → FULL)
